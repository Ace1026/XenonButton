	...
	if (btn->curr_state)
	{
		...
		if (!(btn->flags & BTN_FLAG_ONPRESS_SENT))
		{
			/*
				...
				*/
			if ((xbtn_time_t)(mstime - btn->time_state_change) >= (BTN_CFG_TIME_DEBOUNCE_PRESS / BTN_CFG_TIME_INTERVAL))
			{
			#if BTN_CFG_USE_CLICK
				/*
					* Check mutlti click limit reach or not.
					*/
				if ((btn->click_cnt > 0) && ((xbtn_time_t)(mstime - btn->click_last_time) >= (BTN_CFG_TIME_CLICK_MULTI_MAX / BTN_CFG_TIME_INTERVAL)))
				{
		...
		{
			if ((xbtn_time_t)(mstime - btn->time_change) > (BTN_CFG_TIME_CLICK_PRESS_MAX / BTN_CFG_TIME_INTERVAL))
			{
				if(btn->is_combo == 0)
				{
			...
			while (((BTN_CFG_TIME_KEEPALIVE_PERIOD / BTN_CFG_TIME_INTERVAL) > 0)
				&& ((xbtn_time_t)(mstime - btn->keepalive_last_time) >= (BTN_CFG_TIME_KEEPALIVE_PERIOD / BTN_CFG_TIME_INTERVAL)))
			{
				btn->keepalive_last_time += (BTN_CFG_TIME_KEEPALIVE_PERIOD / BTN_CFG_TIME_INTERVAL);
				btn->keepalive_cnt++;
			...
			if ((btn->click_cnt > 0) && ((xbtn_time_t)(mstime - btn->time_change) > (BTN_CFG_TIME_CLICK_PRESS_MAX / BTN_CFG_TIME_INTERVAL)))
			{
				if (btn->event_mask & BTN_EVT_MASK_ONCLICK)
				{
					if(obj->evt_fn != NULL)
	...
	else
	{
		...
		if (btn->flags & BTN_FLAG_ONPRESS_SENT)
		{
			/*
				...
				*/
			if ((xbtn_time_t)(mstime - btn->time_state_change) >= (BTN_CFG_TIME_DEBOUNCE_RELEASE / BTN_CFG_TIME_INTERVAL))
			{
				...
				if ((xbtn_time_t)(mstime - btn->time_change) >= (BTN_CFG_TIME_CLICK_PRESS_MIN / BTN_CFG_TIME_INTERVAL) &&
					(xbtn_time_t)(mstime - btn->time_change) <= (BTN_CFG_TIME_CLICK_PRESS_MAX / BTN_CFG_TIME_INTERVAL))
				{
					btn->click_cnt++;
				...
				else
				{
					...
					if ((btn->click_cnt > 0) && ((xbtn_time_t)(mstime - btn->time_change) < (BTN_CFG_TIME_CLICK_PRESS_MIN / BTN_CFG_TIME_INTERVAL)))
					{
						if (btn->event_mask & BTN_EVT_MASK_ONCLICK)
						{
							if(obj->evt_fn != NULL)
		...
		else
		{
			...
			if (btn->click_cnt > 0)
			{
				if ((xbtn_time_t)(mstime - btn->click_last_time) >= (BTN_CFG_TIME_CLICK_MULTI_MAX / BTN_CFG_TIME_INTERVAL))
				{
					if (btn->event_mask & BTN_EVT_MASK_ONCLICK)
					{
						if(obj->evt_fn != NULL)
...
static void xbtn_process_all_btn(xbtn_t* obj, xbtn_time_t mstime)
{
	...
	if(obj->btn_process_idx != BTN_IDX_MAX)
	{
		...
		{
			if((mstime - obj->btn[i].time_change) >= (BTN_CFG_TIME_SINGLE_BTN_DELAY / BTN_CFG_TIME_INTERVAL))
			{
	...
	if(obj->cbtn_process_idx != BTN_IDX_MAX)
	{
		...
		{
			if((mstime - obj->cbtn[i].time_change) >= (BTN_CFG_TIME_COMBO_BTN_DELAY / BTN_CFG_TIME_INTERVAL))
			{
	...
	{
		if((mstime - obj->btn_invalid_time) >= (BTN_CFG_TIME_INVALID_BTN_DELAY / BTN_CFG_TIME_INTERVAL))
		{
			obj->btn_process_cnt = 0;
		}
		return;
	...
	if(obj->btn_process_cnt == 2)
	{
		...
		{
			if((mstime - obj->btn_2_process_time) >= 
...
void xbtn_process(void)
{
	...
	obj->btn_run_time++;
	xbtn_process_all_btn(obj, obj->btn_run_time);
}
...
{
	return (xbtn_default.btn_process_cnt != 0);
}

xbtn_time_t xbtn_get_run_time(void)
{
	return xbtn_default.btn_run_time;
}
